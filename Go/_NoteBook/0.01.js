...
3-6 go build 命令简介
	...
	执行该命令且不追加任何参数时，它会试图把当前目录作为代码包编译
...
3-8 go install 命令简介
	...
	安装命令源码文件会在当前工作区的'bin'目录或 $GOBIN 目录下生成'可执行文件'
...
3-10 go get 命令简介
	用于从远程代码仓库（如Github、GitLab）上下载并安装代码包
	接受支持的代码版本控制系统有：Git、Mercuril（hg）、SVN、Bazaar
	指定的代码包会被下载到$GOPATH中包含的第一个工作区的src目录中

3-12 go get 常用标记的使用
	-d : 只执行下载动作，而不执行安装动作
	-fix : 在下载代码包后先执行修正动作，而后再进行编译和安装
		比如将旧版本的代码包修正编译成当前使用的版本
